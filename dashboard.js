...
function aggregateMap(field, key) {
  ...
  DATA.forEach((r) => {
    ;(r[field] ?? []).forEach((e) => {
      m.set(
        e[key] ?? e.name,
        (m.get(e[key] ?? e.name) ?? 0) +
          (e.damage ?? e.count ?? e.total ?? e.ms ?? 0),
      )
    })
  })
...
function calcShame() {
  ...
  DATA.forEach((raid) => {
    ...
    ;(raid.avoidableDamage ?? []).forEach((m) =>
      m.players.forEach((p) =>
        avoidMap.set(p.name, (avoidMap.get(p.name) ?? 0) + p.total),
      ),
    )
    const avoidSorted = [...avoidMap.entries()]
      .map(([name, total]) => ({ name, total }))
      .sort((a, b) => b.total - a.total)
    for (const name of participants) {
      const avg =
        (pct(raid.leaderboard, name) +
          pct(raid.deathStats?.deaths ?? [], name) +
          pct(raid.deathStats?.timeDead ?? [], name) +
          pct(avoidSorted, name)) /
        4
...
function buildResumen() {
  const totalFF = DATA.reduce(
    (s, r) => s + r.leaderboard.reduce((ss, e) => ss + e.damage, 0),
    0,
  )
  const totalKills = DATA.reduce(
    (s, r) => s + (r.bossStats?.totalKills ?? 0),
    0,
  )
  const totalWipes = DATA.reduce(
    (s, r) => s + (r.bossStats?.totalWipes ?? 0),
    0,
  )
  const totalTries = totalKills + totalWipes
  const globalEff =
    totalTries > 0 ? Math.round((totalKills / totalTries) * 100) : 0
  const effColor =
    globalEff >= 80
      ? "var(--gold)"
      : globalEff >= 50
  ...
  const totalDeaths = DATA.reduce(
    (s, r) =>
      s + (r.deathStats?.deaths ?? []).reduce((ss, e) => ss + e.count, 0),
    0,
  )

  const avgDeaths = DATA.length > 0 ? (totalDeaths / DATA.length).toFixed(1) : 0

  let bestDpsRaid = null,
    bestHpsRaid = null
  DATA.forEach((raid) => {
    const s = calcRaidDpsHps(raid)
    if (!s) return
    if (!bestDpsRaid || s.dps > bestDpsRaid.dps)
      bestDpsRaid = { dps: s.dps, fecha: raid.fecha }
    if (!bestHpsRaid || s.hps > bestHpsRaid.hps)
      bestHpsRaid = { hps: s.hps, fecha: raid.fecha }
  })
  ...
  const dMap = new Map()
  DATA.forEach((r) =>
    (r.deathStats?.deaths ?? []).forEach((e) =>
      dMap.set(e.name, (dMap.get(e.name) ?? 0) + e.count),
    ),
  )
  const deathArr = [...dMap.entries()]
    .map(([name, val]) => ({ name, val }))
    .sort((a, b) => b.val - a.val)
    .slice(0, 3)
  ...
  const intMap = new Map(),
    disMap = new Map()
  DATA.forEach((r) => {
    ;(r.interrupts ?? []).forEach((e) =>
      intMap.set(e.name, (intMap.get(e.name) ?? 0) + e.total),
    )
    ;(r.dispels ?? []).forEach((e) =>
      disMap.set(e.name, (disMap.get(e.name) ?? 0) + e.total),
    )
  })
  const intArr = [...intMap.entries()]
    .map(([name, val]) => ({ name, val }))
    .sort((a, b) => b.val - a.val)
    .slice(0, 3)
  const disArr = [...disMap.entries()]
    .map(([name, val]) => ({ name, val }))
    .sort((a, b) => b.val - a.val)
    .slice(0, 3)
  ...
  const recCard = (icon, label, rec, nameFn, targetFn) => {
    if (!rec)
      return `<div class="record-card"><div class="record-icon">${icon}</div><div class="record-label">${label}</div><div class="record-val td-dim">Sin datos</div></div>`
    const abilityLine = rec.ability
      ? `<div class="record-ability">${rec.ability}</div>`
      : ""
  ...
  recEl.innerHTML = `
    ...
    <div class="records-grid" style="grid-template-columns:repeat(3,1fr)">
      ...
      ${recCard(
        "💀",
        "Golpe más bestia recibido",
        records.biggestReceived,
        (r) => r.agresor,
        (r) => r.victima,
      )}
    </div>
...
function calcRaidDpsHps(raid) {
  ...
  return {
    dps: Math.round(totalDmg / totalSec),
    hps: Math.round(totalHeal / totalSec),
  }
}
...
function calcPerformanceRecords() {
  ...
  for (const raid of DATA) {
    if (raid.topDps?.dps > (bestDps?.dps ?? 0))
      bestDps = { ...raid.topDps, fecha: raid.fecha }
    if (raid.topHps?.hps > (bestHps?.hps ?? 0))
      bestHps = { ...raid.topHps, fecha: raid.fecha }
...
function calcGlobalRecords() {
  let biggestHit = null,
    biggestHeal = null,
    biggestReceived = null
  for (const raid of DATA) {
    const bh = raid.biggestHits
    if (!bh) continue
    if (
      bh.biggestDealt?.amount > 0 &&
      (!biggestHit || bh.biggestDealt.amount > biggestHit.amount)
    )
      biggestHit = { ...bh.biggestDealt, fecha: raid.fecha }
    if (
      bh.biggestHeal?.amount > 0 &&
      (!biggestHeal || bh.biggestHeal.amount > biggestHeal.amount)
    )
      biggestHeal = { ...bh.biggestHeal, fecha: raid.fecha }
    if (
      bh.biggestReceived?.amount > 0 &&
      (!biggestReceived || bh.biggestReceived.amount > biggestReceived.amount)
    )
...
function buildPlayerMaps() {
  ...
  PLAYER_SPEC_MAP = {}
  DATA.forEach((r) => {
    Object.entries(r.playerClasses ?? {}).forEach(([n, c]) => {
      if (!PLAYER_CLASS_MAP[n]) PLAYER_CLASS_MAP[n] = c
    })
    Object.entries(r.playerSpecs ?? {}).forEach(([n, s]) => {
      if (!PLAYER_SPEC_MAP[n]) PLAYER_SPEC_MAP[n] = s
    })
  })
}

function getPlayerClass(name) {
  return PLAYER_CLASS_MAP[name] ?? null
}
function getPlayerSpec(name) {
  return PLAYER_SPEC_MAP[name] ?? null
}
...
function progLegend(labels, colors) {
  return `<div class="prog-legend">${labels
    .map(
      (l, i) =>
...
function drawLineChart(xLabels, series, yFormat, yFixedMin, yFixedMax) {
  ...
  if (!allVals.length)
    return '<p class="section-note">Sin datos suficientes.</p>'

  const yMax = yFixedMax != null ? yFixedMax : Math.max(...allVals)
  const yBottom =
    yFixedMin != null ? yFixedMin : Math.max(0, Math.min(...allVals) * 0.8)
  ...
  series.forEach((s) => {
    ...
    if (d)
      seriesSVG += `<path d="${d}" fill="none" stroke="${s.color}" stroke-width="2.5" stroke-linejoin="round" stroke-linecap="round" opacity="0.9"/>`
    s.values.forEach((v, i) => {
      if (v == null) return
      const x = xPos(i).toFixed(1),
        y = yPos(v).toFixed(1)
      const tip = s.label
        ? `${xLabels[i]} · ${s.label}: ${yFormat(v)}`
        : `${xLabels[i]}: ${yFormat(v)}`
...
function drawStackedBar(xLabels, series) {
  ...
  const gridMax =
    yMax <= 4
      ? yMax
      : Math.ceil(yMax / Math.ceil(yMax / 4)) * Math.ceil(yMax / 4)
...
function drawRadarChart(scores, labels) {
  ...
  const playerPts = scores
    .map((s, i) =>
      pt(i, (R * Math.max(0, s)) / 100)
        .map((v) => v.toFixed(1))
        .join(","),
    )
    .join(" ")
  ...
  labels.forEach((l, i) => {
    ...
    if (lines.length > 1) {
      ...
      lines.forEach((ln, li) => {
        labelsSVG += `<tspan x="${x.toFixed(1)}" dy="${li === 0 ? 0 : 12}">${ln}</tspan>`
      })
      labelsSVG += "</text>"
    } else {
...
function buildPorRaid() {
  ...
  if (!DATA.length) {
    el.innerHTML = '<div class="empty-msg">No hay raids registradas.</div>'
    return
  }
  ...
  function miniTableExpand(headers, rows, emptyMsg) {
    ...
    const rowsHTML = rows
      .map((r, i) =>
        i >= VISIBLE
          ? r.replace("<tr", '<tr class="expand-row" style="display:none"')
          : r,
      )
      .join("")
  ...
  function renderRaid(raid) {
    ...
    // ── Boss cards ──
    const bossCards = (raid.dpsStats ?? [])
      .map((b) => {
        const wipes =
          (bs?.bosses ?? []).find((x) => x.name === b.name)?.wipes ?? 0
        const bossIdx = PROG_BOSSES.indexOf(b.name)
        const bossColor =
          bossIdx >= 0 ? PROG_COLORS[bossIdx] : "var(--text-bright)"
    ...
    const timeDeadRows = timeDead.map(
      (e, i) => `<tr>
      <td class="rank-num ${rankClass(i)}">${medalEmoji(i)}</td>
      <td><span class="player-link" data-player="${e.name}">${e.name}</span></td>
    ...
    ;(raid.avoidableDamage ?? []).forEach((m) => {
      m.players.forEach((p) => {
        if (!avoidMapR.has(p.name)) {
          const obj = { total: 0 }
          AVOIDABLE_MECHS_R.forEach((k) => {
            obj[k] = 0
          })
          avoidMapR.set(p.name, obj)
        }
    ...
    const avoidSortedR = [...avoidMapR.entries()]
      .map(([name, d]) => ({ name, ...d }))
      .sort((a, b) => b.total - a.total)
    const avoidSortedSimple = avoidSortedR.map((e) => ({
      name: e.name,
      total: e.total,
    }))
          ...
          <tbody>${avoidSortedR
            .map(
              (
                e,
                i,
              ) => `<tr${i >= 5 ? ' class="expand-row" style="display:none"' : ""}>
            <td class="rank-num ${rankClass(i)}">${medalEmoji(i)}</td>
            <td><span class="player-link" data-player="${e.name}">${e.name}</span></td>
            ...
            .join(
              "",
            )}${avoidSortedR.length > 5 ? `<tr class="expand-toggle-row" style="cursor:pointer"><td colspan="${2 + AVOIDABLE_MECHS_R.length + 1}" style="text-align:center;padding:.5rem .4rem;color:var(--text-dim);font-size:.82rem;user-select:none">▼ Ver todos (${avoidSortedR.length})</td></tr>` : ""}</tbody>
        </table>
      </div>`
        })()
    ...
    const mvp =
      allShameScores.length > 1
        ? allShameScores[allShameScores.length - 1]
        : null
    const mvpHazHTML =
      mvp && hazmerreir && hazmerreir.name !== mvp.name
        ? `
    ...
    for (const bossEntry of raid.tankMitigations ?? []) {
      ...
      if (best && best.pct > (topMit?.pct ?? 0)) {
        topMit = best
        topMitBoss = bossEntry.bossName ?? ""
      }
    }
    ...
    const topCards = [
      ...
      bh?.biggestDealt
        ? nightCard(
            "⚔️",
            "Golpe más fuerte",
            bh.biggestDealt.heroe,
            fmtDmg(bh.biggestDealt.amount),
            "→ " +
              bh.biggestDealt.objetivo +
              (bh.biggestDealt.ability
                ? ` · <span style="color:var(--purple2);font-style:italic">${bh.biggestDealt.ability}</span>`
                : ""),
            "",
          )
        : "",
      bh?.biggestHeal
        ? nightCard(
            "💚",
            "Cura más gorda",
            bh.biggestHeal.healer,
            fmtDmg(bh.biggestHeal.amount),
            "→ " +
              bh.biggestHeal.target +
              (bh.biggestHeal.ability
                ? ` · <span style="color:var(--purple2);font-style:italic">${bh.biggestHeal.ability}</span>`
                : ""),
            "",
          )
        : "",
      bh?.biggestReceived
        ? nightCard(
            "💀",
            "Golpe más bestia recibido",
            bh.biggestReceived.victima,
            fmtDmg(bh.biggestReceived.amount),
            "← " +
              bh.biggestReceived.agresor +
              (bh.biggestReceived.ability
                ? ` · <span style="color:var(--purple2);font-style:italic">${bh.biggestReceived.ability}</span>`
                : ""),
            "",
          )
        : "",
    ...
    const dpsFilt = dpsArr.filter(
      (e) => !HEALER_SPECS.has(PLAYER_SPEC_MAP[e.name]),
    )
    const hpsFilt = hpsArr.filter((e) =>
      HEALER_SPECS.has(PLAYER_SPEC_MAP[e.name]),
    )
    ...
    const perfRankHTML =
      dpsRankRows.length || hpsRankRows.length || mitRankRows.length
        ? `
      <div class="two-col" style="margin-bottom:2rem">
        <div>
  ...
  document.getElementById("raid-selector").addEventListener("change", (e) => {
    ...
    document.getElementById("wcl-link").href =
      `https://www.warcraftlogs.com/reports/${raid.report}`
    renderRaid(raid)
  })
}
  ...
  const dpsSeries = [
    {
      label: "DPS",
      color: DPS_COLOR,
      values: raidStats.map((s) => s?.dps ?? null),
    },
    {
      label: "HPS",
      color: HPS_COLOR,
      values: raidStats.map((s) => s?.hps ?? null),
    },
  ]
  return `
...
function buildDpsHpsTable(raids, xLabels) {
  ...
  const bossNames = [
    ...new Set(raids.flatMap((r) => (r.dpsStats ?? []).map((b) => b.name))),
  ]
  const shortName = (n) => {
    if (n.includes("Maulgar")) return "Maulgar"
    if (n.includes("Gruul")) return "Gruul"
  ...
  const dpsRows = raids.map((raid, i) => {
    const cells = bossNames
      .map((bn) => {
        const b = (raid.dpsStats ?? []).find((b) => b.name === bn)
        return b
          ? `<td class="val-cell" style="color:${BOSS_VAL_COLOR}">${fmtDmg(b.dps)}</td>`
          : `<td class="td-dim">—</td>`
      })
      .join("")
  ...
  const hpsRows = raids.map((raid, i) => {
    const cells = bossNames
      .map((bn) => {
        const b = (raid.dpsStats ?? []).find((b) => b.name === bn)
        return b
          ? `<td class="val-cell" style="color:${BOSS_VAL_COLOR}">${fmtDmg(b.hps)}</td>`
          : `<td class="td-dim">—</td>`
      })
      .join("")
    const total = raidStats[i]
  ...
  const bossHeaders = bossNames
    .map((bn) => `<th>${shortName(bn)}</th>`)
    .join("")
...
function buildProgresion() {
  const raids = DATA.filter((r) => r.bossStats).sort((a, b) =>
    a.fecha.localeCompare(b.fecha),
  )
  const el = document.getElementById("tab-progresion")
  if (!raids.length) {
    el.innerHTML =
      '<div class="section-note">No hay datos de progresión disponibles.</div>'
    return
  }
  ...
  raids.forEach((r) =>
    (r.bossStats.bosses ?? []).forEach((b) =>
      wipesByBoss.set(b.name, (wipesByBoss.get(b.name) ?? 0) + b.wipes),
    ),
  )
  const sortedByWipes = [...wipesByBoss.entries()].sort((a, b) => b[1] - a[1])
  const [worstBossName, worstBossWipes] = sortedByWipes[0] ?? [null, 0]
  const [bestBossName, bestBossWipes] = sortedByWipes[
    sortedByWipes.length - 1
  ] ?? [null, 0]
  const worstBossIdx = PROG_BOSSES.indexOf(worstBossName)
  const bestBossIdx = PROG_BOSSES.indexOf(bestBossName)
  const worstBossShort =
    worstBossIdx >= 0 ? PROG_SHORT[worstBossIdx] : (worstBossName ?? "—")
  const bestBossShort =
    bestBossIdx >= 0 ? PROG_SHORT[bestBossIdx] : (bestBossName ?? "—")
  const worstBossColor =
    worstBossIdx >= 0 ? PROG_COLORS[worstBossIdx] : "var(--text-bright)"
  const bestBossColor =
    bestBossIdx >= 0 ? PROG_COLORS[bestBossIdx] : "var(--text-bright)"
  ...
  const raidTimeSeries = [
    {
      ...
      color: "#f0c84a",
      values: raids.map((r) =>
        r.bossStats.totalRaidTimeMs > 0
          ? r.bossStats.totalRaidTimeMs / 60000
          : null,
      ),
    },
  ]
  ...
  const wipeSeries = PROG_BOSSES.map((bossName, i) => ({
    ...
    values: raids.map(
      (r) =>
        (r.bossStats.bosses ?? []).find((b) => b.name === bossName)?.wipes ?? 0,
    ),
  }))
  ...
  const bossRows = PROG_BOSSES.map((bossName, i) => {
    ...
    const attempted = raids.filter((r) =>
      (r.bossStats.bosses ?? []).some((b) => b.name === bossName),
    ).length
...
function buildVerguenza() {
  ...
  DATA.forEach((raid) => {
    ...
    ;(raid.avoidableDamage ?? []).forEach((m) =>
      m.players.forEach((p) =>
        avoidMap2.set(p.name, (avoidMap2.get(p.name) ?? 0) + p.total),
      ),
    )
    const avoidSorted2 = [...avoidMap2.entries()]
      .map(([name, total]) => ({ name, total }))
      .sort((a, b) => b.total - a.total)
    for (const name of participants) {
      const score =
        (pct(raid.leaderboard, name) +
          pct(raid.deathStats?.deaths ?? [], name) +
          pct(raid.deathStats?.timeDead ?? [], name) +
          pct(avoidSorted2, name)) /
        4
      if (!worstRaid || score > worstRaid.score)
        worstRaid = { name, score, fecha: raid.fecha }
      if (!bestRaid || score < bestRaid.score)
        bestRaid = { name, score, fecha: raid.fecha }
    }
  })
  ...
  // Sub-tab switching
  document
    .querySelectorAll("#sub-nav-verguenza .sub-tab-btn")
    .forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        document
          .querySelectorAll("#tab-verguenza .sub-tab-content")
          .forEach((c) => c.classList.remove("active"))
        btn.classList.add("active")
        document
          .getElementById("subtab-" + btn.dataset.subtab)
...
function buildMuertes() {
  ...
  DATA.forEach((r) => {
    ...
    ;(r.deathStats?.timeDead ?? []).forEach((e) =>
      tMap.set(e.name, (tMap.get(e.name) ?? 0) + e.ms),
    )
    const f = r.deathStats?.firstToDie?.name
    if (f) fMap.set(f, (fMap.get(f) ?? 0) + 1)
  })

  const deaths = [...dMap.entries()]
    .map(([name, val]) => ({ name, val }))
    .sort((a, b) => b.val - a.val)
  const tdead = [...tMap.entries()]
    .map(([name, val]) => ({ name, val }))
    .sort((a, b) => b.val - a.val)
  const first = [...fMap.entries()]
    .map(([name, val]) => ({ name, val }))
    .sort((a, b) => b.val - a.val)
  ...
  const mkTable = (id, arr, valFn, cls) => {
    ...
    el.querySelectorAll(".player-link").forEach((el2) =>
      el2.addEventListener("click", () => openPlayer(el2.dataset.player)),
    )
  }
  ...
  fd.querySelectorAll(".player-link").forEach((el) =>
    el.addEventListener("click", () => openPlayer(el.dataset.player)),
  )
}
...
function buildMecanicas() {
  ...
  const ints = [...intMap.entries()]
    .map(([name, val]) => ({ name, val }))
    .sort((a, b) => b.val - a.val)
  const disp = [...disMap.entries()]
    .map(([name, val]) => ({ name, val }))
    .sort((a, b) => b.val - a.val)
  ...
  const mkTable = (id, arr) => {
    const el = document.getElementById(id)
    if (!arr.length) {
      el.innerHTML = '<tr><td colspan="5" class="empty-msg">Sin datos</td></tr>'
      return
    }
...
function buildAvoidable() {
  ...
  const mediaAvoidRaid =
    DATA.length > 0
      ? DATA.reduce(
          (s, r) =>
            s +
            (r.avoidableDamage ?? []).reduce(
              (ss, m) => ss + m.players.reduce((sss, p) => sss + p.total, 0),
              0,
            ),
          0,
        ) / DATA.length
  ...
  const raidMaxAvoid = DATA.reduce((best, r) => {
    const sum = (r.avoidableDamage ?? []).reduce(
      (s, m) => s + m.players.reduce((ss, p) => ss + p.total, 0),
      0,
    )
    return sum > (best.sum ?? 0) ? { sum, fecha: r.fecha } : best
  ...
  const topPlayer =
    allRows.length > 0
      ? allRows.reduce(
          (best, e) => (e.total > (best?.total ?? 0) ? e : best),
          null,
        )
      : null
...
function buildHistorial() {
  ...
  if (!DATA.length) {
    container.innerHTML =
      '<div class="empty-msg">No hay raids registradas.</div>'
    return
  }
  ...
  const bestByBoss = {
    "High King Maulgar": minTime(
      (r) =>
        (r.bossStats?.bosses ?? []).find((b) => b.name === "High King Maulgar")
          ?.killTimeMs || null,
    ),
    "Gruul the Dragonkiller": minTime(
      (r) =>
        (r.bossStats?.bosses ?? []).find(
          (b) => b.name === "Gruul the Dragonkiller",
        )?.killTimeMs || null,
    ),
    Magtheridon: minTime(
      (r) =>
        (r.bossStats?.bosses ?? []).find((b) => b.name === "Magtheridon")
          ?.killTimeMs || null,
    ),
  }
  ...
  const rows = sortedRaids
    .map((r, raidIdx) => {
      ...
      const effVal =
        bs?.totalTries > 0
          ? Math.round((bs.totalKills / bs.totalTries) * 100)
          : null
      const effColor =
        effVal === null
          ? "var(--text-dim)"
...
function buildTimeRecords() {
  ...
  const fastest = withTime.reduce((a, b) =>
    a.bossStats.totalRaidTimeMs < b.bossStats.totalRaidTimeMs ? a : b,
  )
  ...
  const fastestByBoss = Object.entries(BOSS_LABELS)
    .map(([name, { label, icon }]) => {
      ...
      return best
        ? { label, icon, killTimeMs: best.killTimeMs, fecha: best.fecha }
        : null
    })
    .filter(Boolean)
...
function calcTitulos() {
  ...
  const avoidByMech = {
    ...
    "Cave In": new Map(),
    Debris: new Map(),
    Conflagration: new Map(),
  }

  DATA.forEach((raid) => {
    ;(raid.leaderboard ?? []).forEach((e) =>
      ffTotals.set(e.name, (ffTotals.get(e.name) ?? 0) + e.damage),
    )
    ;(raid.deathStats?.deaths ?? []).forEach((e) =>
      deathTotals.set(e.name, (deathTotals.get(e.name) ?? 0) + e.count),
    )
    ;(raid.deathStats?.timeDead ?? []).forEach((e) =>
      timeDeadTotals.set(e.name, (timeDeadTotals.get(e.name) ?? 0) + e.ms),
    )
    if (raid.leaderboard[0])
      portadorCount.set(
        raid.leaderboard[0].name,
        (portadorCount.get(raid.leaderboard[0].name) ?? 0) + 1,
      )
    ;(raid.dispels ?? []).forEach((e) =>
      dispelTotals.set(e.name, (dispelTotals.get(e.name) ?? 0) + e.total),
    )
    ;(raid.interrupts ?? []).forEach((e) =>
      intTotals.set(e.name, (intTotals.get(e.name) ?? 0) + e.total),
    )
    ;(raid.avoidableDamage ?? []).forEach((mech) => {
      mech.players.forEach((p) => {
        avoidTotal.set(p.name, (avoidTotal.get(p.name) ?? 0) + p.total)
        if (mech.mechanic in avoidByMech)
          avoidByMech[mech.mechanic].set(
            p.name,
            (avoidByMech[mech.mechanic].get(p.name) ?? 0) + p.total,
          )
      })
    })
  })
  ...
  // ── Vergüenza ──
  const pickFor = (arr) => arr[Math.floor(Math.random() * arr.length)]
  ...
  if (topAvoid)
    titles.push({
      ...
      icon: "💥",
      titulo: "El Kamikaze",
      desc: "Más daño recibido de mecánicas evitables",
      jugador: topAvoid.name,
      comentario: pickFor([
  ...
  if (topBlast)
    titles.push({
      ...
      icon: "🔥",
      titulo: "El Temerario",
      desc: "Más daño de Blast Wave · demasiado cerca de Krosh",
      jugador: topBlast.name,
      comentario: pickFor([
  ...
  if (topWhirl)
    titles.push({
      ...
      icon: "🌀",
      titulo: "El Picadora",
      desc: "Más daño de Whirlwind en Maulgar",
      jugador: topWhirl.name,
      comentario: pickFor([
  ...
  if (topCaveIn)
    titles.push({
      id: "estalactita",
      icon: "🪨",
      titulo: "El Estalactita",
      desc: "Más daño de Cave In · rocas de Gruul",
      jugador: topCaveIn.name,
      comentario: pickFor([
  ...
  if (topConf)
    titles.push({
      ...
      icon: "🍖",
      titulo: "El Tostado",
      desc: "Más daño de Conflagration · fuego en el suelo",
      jugador: topConf.name,
      comentario: pickFor([
  ...
  if (topFF)
    titles.push({
      ...
      icon: "🗡️",
      titulo: "El Traidor",
      desc: "Más daño total a aliados histórico",
      jugador: topFF.name,
      comentario: pickFor([
      ...
      valor: fmtDmg(topFF.val) + " de FF total",
      tipo: "shame",
    })
  ...
  if (topDead)
    titles.push({
      ...
      icon: "💀",
      titulo: "El Mártir",
      desc: "Más muertes históricas",
      jugador: topDead.name,
      comentario: pickFor([
  ...
  if (topGhost)
    titles.push({
      ...
      icon: "👻",
      titulo: "El Fantasma",
      desc: "Mayor tiempo muerto histórico",
      jugador: topGhost.name,
      comentario: pickFor([
  ...
  DATA.forEach((raid) => {
    ...
    if (
      br?.amount > (masoca?.amount ?? 0) &&
      (rcMap.get(br.victima) ?? 0) >= minRaids
    )
      masoca = { ...br, fecha: raid.fecha }
  })
  if (masoca)
    titles.push({
      id: "masoca",
      icon: "🤕",
      titulo: "El Masoca",
      desc: "Mayor golpe recibido en toda la temporada",
      jugador: masoca.victima,
      comentario: pickFor([
      ...
      valor:
        fmtDmg(masoca.amount) + (masoca.ability ? ` · ${masoca.ability}` : ""),
      tipo: "shame",
    })
  ...
  const capable = Object.entries(PLAYER_CLASS_MAP)
    .filter(([n, cls]) => {
      if (
        !CAPABLE_CLASS.has(cls) &&
        !(cls === "Shaman" && SHAMAN_CAPABLE.has(PLAYER_SPEC_MAP[n]))
      )
        return false
  ...
  if (capable.length > 0) {
    ...
    titles.push({
      ...
      icon: "🤐",
      titulo: "El Mudo",
      desc: "Menos interrupts entre los capaces",
      jugadores: mudos,
      comentario: pickFor([
  ...
  if (topDisp)
    titles.push({
      ...
      icon: "🧿",
      titulo: "El Escudo",
      desc: "Más dispels históricos",
      jugador: topDisp.name,
      comentario: pickFor([
  ...
  if (topInt)
    titles.push({
      ...
      icon: "⚡",
      titulo: "El Centinela",
      desc: "Más interrupts históricos",
      jugador: topInt.name,
      comentario: pickFor([
  ...
  if (survivors.length > 0)
    titles.push({
      id: "superviviente",
      icon: "🌿",
      titulo: "El Superviviente",
      desc: "Menos muertes por raid entre los asiduos",
      jugador: survivors[0].name,
      comentario: pickFor([
      ...
      valor: survivors[0].val.toFixed(2) + " muertes/raid",
      tipo: "honor",
    })
  ...
  const pacifistas = [...rcMap.entries()]
    ...
    .sort((a, b) => b[1] - a[1])
  if (pacifistas.length > 0)
    titles.push({
      id: "pacifista",
      icon: "☮️",
      titulo: "El Pacifista",
      desc: "Nunca ha dañado a un aliado",
      jugador: pacifistas[0][0],
      comentario: pickFor([
      ...
      valor: rcMap.get(pacifistas[0][0]) + " raids sin FF",
      tipo: "honor",
    })
  ...
  DATA.forEach((raid) => {
    const deadInRaid = new Set(
      (raid.deathStats?.deaths ?? [])
        .filter((e) => e.count > 0)
        .map((e) => e.name),
    )
    ;(raid.roster ?? []).forEach((name) => {
      if (!deadInRaid.has(name))
        spartanCount.set(name, (spartanCount.get(name) ?? 0) + 1)
    })
  })
  const topSpartan = [...spartanCount.entries()]
    .filter(([n]) => (rcMap.get(n) ?? 0) >= minRaids)
    .sort((a, b) => b[1] - a[1])
  if (topSpartan.length > 0)
    titles.push({
      id: "espartano",
      icon: "🏛️",
      titulo: "El Espartano",
      desc: "Más raids sin morir ni una vez",
      jugador: topSpartan[0][0],
      comentario: pickFor([
      ...
      valor: topSpartan[0][1] + " raids limpias",
      tipo: "honor",
    })
  ...
  if (intocables.length > 0)
    titles.push({
      ...
      icon: "🧘",
      titulo: "El Intocable",
      desc: "Menos daño recibido de mecánicas evitables",
      jugador: intocables[0].name,
      comentario: pickFor([
  ...
  DATA.forEach((raid) => {
    Object.entries(raid.playerHitStats ?? {}).forEach(([pname, stats]) => {
      if (
        stats.biggestHit?.amount > (verdugo?.amount ?? 0) &&
        (rcMap.get(pname) ?? 0) >= minRaids
      )
  ...
  if (verdugo)
    titles.push({
      ...
      icon: "🗡️",
      titulo: "El Verdugo",
      desc: "Mayor golpe único infligido en toda la temporada",
      jugador: verdugo.jugador,
      comentario: pickFor([
      ...
      valor:
        fmtDmg(verdugo.amount) +
        (verdugo.ability ? ` · ${verdugo.ability}` : "") +
        (verdugo.target ? ` → ${verdugo.target}` : ""),
      tipo: "honor",
    })
  ...
  // El Exterminador: mayor DPS acumulado en todos los boss kills históricos
  {
    ...
    let extRecord = null
    allDps.forEach((v, name) => {
      const dps = Math.round(v.total / v.time)
      if (dps > (extRecord?.dps ?? 0) && (rcMap.get(name) ?? 0) >= minRaids)
        extRecord = { name, dps }
    })
    if (extRecord)
      titles.push({
        ...
        valor: fmtDmg(extRecord.dps) + " DPS",
        tipo: "honor",
      })
  }
  ...
  // El Aerith: mayor HPS acumulado en todos los boss kills históricos
  {
    ...
    allHps.forEach((v, name) => {
      const hps = Math.round(v.total / v.time)
      if (hps > (aerithRecord?.hps ?? 0) && (rcMap.get(name) ?? 0) >= minRaids)
        aerithRecord = { name, hps }
    })
    if (aerithRecord)
      titles.push({
        id: "aerith",
        ...
        valor: fmtDmg(aerithRecord.hps) + " HPS",
        tipo: "honor",
      })
  }
  ...
  // El Muro: tanque con mayor % de mitigación acumulada en todos los boss kills históricos
  {
    ...
    DATA.forEach((raid) =>
      (raid.globalMitigation ?? []).forEach((p) => {
        ...
        allMitig.set(p.name, {
          reduced: c.reduced + p.reduced,
          gross: c.gross + (p.gross ?? Math.round(p.reduced / (p.pct / 100))),
        })
      }),
    )
    let muroRecord = null
    allMitig.forEach((v, name) => {
      const pct = Math.round((v.reduced / v.gross) * 1000) / 10
      if (pct > (muroRecord?.pct ?? 0) && (rcMap.get(name) ?? 0) >= minRaids)
        muroRecord = { name, pct, reduced: v.reduced }
    })
    if (muroRecord)
      titles.push({
        id: "guardian",
        icon: "🛡️",
        titulo: "El Muro",
        desc: "Mayor media de daño mitigado en todos los boss kills",
        jugador: muroRecord.name,
        comentario: pickFor([
...
function getPlayerTitles(name) {
  return TITULOS.filter(
    (t) => t.jugador === name || (t.jugadores ?? []).includes(name),
  )
}
...
function buildGaleriaInfamia() {
  ...
  if (!el) return
  if (!TITULOS.length) {
    el.innerHTML = '<div class="empty-msg">Sin datos suficientes.</div>'
    return
  }
  ...
  const card = (t) => {
    ...
    const playerHTML = players
      .map((p) => `<span class="player-link" data-player="${p}">${p}</span>`)
      .join(", ")
    return `
  ...
  el.querySelectorAll(".player-link").forEach((e) =>
    e.addEventListener("click", () => openPlayer(e.dataset.player)),
  )
}
...
function generarTitulares(raid, allRaids) {
  ...
  const raidIdx = sorted.findIndex(
    (r) => r.report === raid.report && r.fightId === raid.fightId,
  )
  const prev = sorted.slice(0, raidIdx)
  const lines = []

  // Seed determinista por raid para elegir variante de texto
  const seed = [...(raid.report ?? "x")].reduce(
  ...
  const totalDeaths = (raid.deathStats?.deaths ?? []).reduce(
    (s, e) => s + e.count,
    0,
  )
  const dps = calcRaidDpsHps(raid)
  ...
  if (winner) {
    let streak = 0
    for (let i = raidIdx; i >= 0; i--) {
      if (sorted[i].leaderboard[0]?.name === winner) streak++
      else break
    }
    if (streak >= 4)
      lines.push(
  ...
  if (bs) {
    if (bs.totalWipes === 0) {
      let cleanStreak = 0
      for (let i = raidIdx; i >= 0; i--) {
        if ((sorted[i].bossStats?.totalWipes ?? 1) === 0) cleanStreak++
        else break
      }
      if (cleanStreak >= 3)
        lines.push(
  ...
  if (bs && prev.length > 0) {
    ...
    for (const boss of thisBosses) {
      ...
      const sn = boss.name
        .replace("Gruul the Dragonkiller", "Gruul")
        .replace("High King Maulgar", "Maulgar")
      const prevTimes = prev
        .map(
          (r) =>
            (r.bossStats?.bosses ?? []).find((b) => b.name === boss.name)
              ?.killTimeMs,
        )
        .filter(Boolean)
      ...
      if (boss.killTimeMs < prevBest) {
        ...
        if (!bestRecord || pct > bestRecord.pct)
          bestRecord = {
            bossName: sn,
            thisMs: boss.killTimeMs,
            prevBestMs: prevBest,
            pct,
          }
      }
  ...
  if (dps && prev.length > 0) {
    const prevBest = prev.reduce((best, r) => {
      const s = calcRaidDpsHps(r)
      return s && s.dps > best ? s.dps : best
    }, 0)
    if (prevBest > 0 && dps.dps > prevBest)
      lines.push(
        pick([
  ...
  ;(raid.avoidableDamage ?? []).forEach((m) =>
    m.players.forEach((p) =>
      avoidMapT.set(p.name, (avoidMapT.get(p.name) ?? 0) + p.total),
    ),
  )
...
function setupJugador() {
  ...
  input.addEventListener("input", () => {
    ...
    sugg.innerHTML = matches
      .map((p) => `<div class="suggestion-item" data-player="${p}">${p}</div>`)
      .join("")
...
function openPlayer(name) {
  ...
  document
    ...
    .forEach((b) => b.classList.toggle("active", b.dataset.tab === "jugador"))
  document
    .querySelectorAll(".tab-content")
    .forEach((c) => c.classList.toggle("active", c.id === "tab-jugador"))
  ...
  const raidsAttended = DATA.filter((r) =>
    (
      r.roster ?? [
        ...r.leaderboard.map((e) => e.name),
        ...(r.deathStats?.deaths ?? []).map((e) => e.name),
  ...
  const rows = raidsAttended.map((r) => {
    ...
    const avoid = (r.avoidableDamage ?? []).reduce(
      (s, m) => s + (m.players.find((p) => p.name === name)?.total ?? 0),
      0,
    )
    totalAvoid += avoid
    totalFF += ff
    ...
    if (n > 1) {
      ...
      const avoidSortedS = [
        ...(() => {
          const m2 = new Map()
          ;(r.avoidableDamage ?? []).forEach((m) =>
            m.players.forEach((p) =>
              m2.set(p.name, (m2.get(p.name) ?? 0) + p.total),
            ),
          )
          return [...m2.entries()]
            .map(([n2, t]) => ({ name: n2, total: t }))
            .sort((a, b) => b.total - a.total)
        })(),
      ]
      shameScore =
        ((pct(r.leaderboard) +
          pct(r.deathStats?.deaths ?? []) +
          pct(r.deathStats?.timeDead ?? []) +
          pct(avoidSortedS)) /
          4) *
        100
    }
  ...
  for (const r of rows) {
    ...
    if (!h) continue
    if (
      h.biggestHit?.amount > 0 &&
      (!prHit || h.biggestHit.amount > prHit.amount)
    )
      prHit = { ...h.biggestHit, fecha: r.fecha }
    if (
      h.biggestHeal?.amount > 0 &&
      (!prHeal || h.biggestHeal.amount > prHeal.amount)
    )
      prHeal = { ...h.biggestHeal, fecha: r.fecha }
    if (
      h.biggestReceived?.amount > 0 &&
      (!prReceived || h.biggestReceived.amount > prReceived.amount)
    )
  ...
  const clsColor = cls
    ? (CLASS_COLOR[cls] ?? "var(--text-bright)")
    : "var(--text-bright)"
  ...
  profile.innerHTML = `
    ...
    <div class="records-grid" style="margin-bottom:1.5rem${prPerf ? ";grid-template-columns:repeat(4,1fr)" : ""}">
      ${prPerf ? `<div class="record-card"><div class="record-icon">${isHealer ? "💊" : "⚡"}</div><div class="record-label">Mejor ${isHealer ? "HPS" : "DPS"}</div><div class="record-amount">${fmtDmg(prPerf.value)} ${isHealer ? "HPS" : "DPS"}</div>${prPerf.bossName ? `<div class="record-ability" style="color:var(--text-dim)">${prPerf.bossName}</div>` : ""}<div class="record-date">${fmtDate(prPerf.fecha)}</div></div>` : ""}
    ...
    ${(() => {
      ...
      const sorted = [...raidsAttended].sort((a, b) =>
        a.fecha.localeCompare(b.fecha),
      )
      ...
      const shameValues = sorted.map((r) => {
        ...
        ;(r.avoidableDamage ?? []).forEach((m) =>
          m.players.forEach((p) =>
            avoidMap3.set(p.name, (avoidMap3.get(p.name) ?? 0) + p.total),
          ),
        )
        const avoidSorted3 = [...avoidMap3.entries()]
          .map(([n2, total]) => ({ name: n2, total }))
          .sort((a, b) => b.total - a.total)
        return (
          ((pct(r.leaderboard) +
            pct(r.deathStats?.deaths ?? []) +
      ...
      const series = [
        { label: "Vergüenza", color: "var(--red2)", values: shameValues },
      ]
      return `
...
function itemIcon(id) {
  ...
  const src = icon
    ? `https://wow.zamimg.com/images/wow/icons/small/${icon}.jpg`
    : ""
...
function _calcMimado() {
  ...
  TITULOS.push({
    ...
    icon: "💸",
    titulo: "El Mimado",
    desc: "Más ítems de loot recibidos",
    jugadores: mimados,
    comentario:
      _mimadoComments[Math.floor(Math.random() * _mimadoComments.length)],
...
function fetchLootData() {
  ...
  window.__lootCallback = function (json) {
    ...
    try {
      ...
      const idx = {}
      cols.forEach((c, i) => {
        if (c.label) idx[c.label] = i
      })
      const get = (row, label) => {
        const i = idx[label]
        return i !== undefined && row.c[i] ? row.c[i].v : null
      }
      lootRows = (json.table.rows || [])
        .filter((r) => r && r.c)
        .map((row) => {
...
function buildLootResumen() {
  ...
  lootRows.forEach((r) => {
    if (!byPlayer.has(r.nombre))
      byPlayer.set(r.nombre, {
        bis: 0,
        upgrade: 0,
        offspec: 0,
        total: 0,
  ...
  const rows = [...byPlayer.entries()]
    .map(([name, s]) => ({ name, ...s }))
    .sort((a, b) => b.bis - a.bis || b.total - a.total)
...
function renderLootPlayer(nombre) {
  ...
  el.innerHTML = `
    <div class="loot-chips">
      ...
      ${
        hasBag
          ? `<div class="loot-chip" style="border-color:var(--gold-dim)">
        <div class="chip-val">${
          bagRow?.itemID
...
document.getElementById("btn-export-png").addEventListener("click", () => {
  ...
  const target =
    activeTab?.id === "tab-por-raid" && porRaidContent
      ? porRaidContent
      : activeTab
  if (!target) return
  ...
  const tabLabel = (activeBtn?.textContent ?? "tab")
    .trim()
    .toLowerCase()
    .replace(/\s+/g, "-")
    .replace(/[^a-z0-9-]/g, "")
...
;(function initChangelog() {
  ...
  entries.innerHTML = CHANGELOG.map((entry, i) => {
    const id = `cl-${i}`
    const sectionsHTML = entry.secciones
      .map(
        (s) => `
  ...
  timeline.innerHTML = CHANGELOG.map(
    (entry, i) => `<a href="#cl-${i}" data-idx="${i}">${entry.fecha}</a>`,
  ).join("")
  ...
  entries.addEventListener("scroll", () => {
    ...
    entryEls.forEach((el, i) => {
      if (el.offsetTop - entries.scrollTop <= 20) current = i
    })
...
;(function () {
  ...
  function place(e) {
    ...
    const rw = tip.offsetWidth,
      rh = tip.offsetHeight
    tip.style.left =
      (x + rw > window.innerWidth ? e.clientX - rw - 8 : x) + "px"
    tip.style.top =
      (y + rh > window.innerHeight ? e.clientY - rh - 8 : y) + "px"
  }
